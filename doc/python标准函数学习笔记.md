...
以上实例声明了一个静态方法f，类可以不用实例化就可以调用该方法C.f()                                    
####64、str(object='')
将对象转化为适于人阅读的形式                        
**参数**                        
object -- 对象                     
...
**返回值**                                     
计算结果
####65、super(type[, object-or-type])
调用父类的一个方法。用来解决多重继承的问题                                
**参数**                               
type -- 类。                          
...
将列表转换为元组                                     
**参数**                           
iterable -- 要转换为元组的序列，可迭代                                           
**返回值**                    
返回元组
####67、type(object)
####　　type(name, bases, dict)
如果只有第一个参数则返回对象的类型，如果有三个参数则返回新的类型对象                  
**参数**                           
name -- 类的名称                            
bases -- 基类的元组                             
dict -- 字典，类内定义的命名空间变量                                 
...
object -- 对象                      
**返回值**                           
返回对象的属性和属性值的字典对象，如果没有参数，就打印当前调用位置的属性和属性值       
####70、xrange(stop)
####　　xrange(start, stop[, step])
与range()完全相同，不同的是生成的不是一个数组，而是一个生成器              
**参数**                                   
start -- 计数从start开始，默认从0开始                          
stop -- 计数到stop结束，但不包括stop                   
step -- 步长，默认为1；                           
...
返回生成器
####71、zip([iterable, ...])
将可迭代的对象作为参数，将对象中对应的元素打包成一个个元组，然后返回由这些元组组成的列表，如果各个迭代器的元素个数不一致，则返回列表长度和最短的对象相同。                     
**参数**                     
iterable -- 一个或多个迭代器                                  
**返回值**                               
元组列表